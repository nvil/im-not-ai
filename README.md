...
## v1.3 — 서브 패턴 발굴 운영 체계 + 본진 신규 1건 (2026-04-25)

**에이전트들이 실전에서 만난 미분류 의심 패턴을 단일 풀에 누적·점검·승격하는 운영 인프라**를 도입하고, 발행 전 파일럿 회차에서 본진 신규 1건(`C-9` 숫자 괄호 인덱싱)·본진 보강 1건(`I-2` 결합형 변종)을 동시에 반영했습니다. v1.1까지는 패턴 추가가 사람의 1회성 결정이었고, v1.2에서는 voice profile 권한 위계가 들어왔다면, v1.3은 **분류 체계 자체가 시간을 따라 자라나는 구조**를 만듭니다.
...
합성 샘플 2건(제조업 칼럼톤·핀테크 리포트톤) 투입 → detector 시뮬레이션이 본진 미분류 의심 후보 3건 발견 → 6게이트 통과 결과:

| 후보 | 결과 | 사유 |
|------|------|------|
| 숫자 괄호 인덱싱 1) 2) 3) | **promoted → C-9** | 6게이트 모두 통과, 본진 신규 등재 |
| 메타 진입 '~을 살펴보면 / 들여다보면' | **hold** | Gate 1.3 (장르 분산) — 같은 합성 회차 출처라 다음 회차 재판정 |
| 'X은 ~라는 점에 있다' 강조 위치 서술 | **merged → I-2 보강** | Gate 2.2 (본진 변종) — 본진 I-2 시그니처 예문에 결합형 4건 흡수 |

v1.2 워크플로였다면 세 후보 모두 reviewer JSON의 `unclassified_candidates` 필드에 기록됐다가 run 종료 후 묻혔을 정보입니다. v1.3에서는 그중 1건이 본진 신규로, 1건이 본진 보강으로 영구 반영됐습니다. 회차별 점검 로그는 운영 산출물이라 `_workspace/taxonomy_changelog.md`에 누적되며 (gitignored), 본진 변경은 [`ai-tell-taxonomy.md` 버전 관리](.claude/skills/humanize-korean/references/ai-tell-taxonomy.md#버전-관리)에 영구 기록됩니다.
