...
```
humanize-korean v1.3 — voice profile {미주입|주입(<author-context.yaml 경로>)} 모드 / run_id: {YYYY-MM-DD-NNN}
```

- 버전(`v1.3`)은 SKILL.md frontmatter `version` 필드를 그대로 노출. 사용자가 어떤 버전이 동작 중인지 첫 응답에서 즉시 확인할 수 있게 한다.
