...
3. **항목별 판정** (각 후보에 대해):
   - `occurrences ≥ 2` 충족? 미달이면 `hold`(다음 회차 재검토) 또는 `rejected: single_run_only`(생성 90일+ 미재현 시)
   - 장르 분산 충족? 같은 작가·같은 run에서만 재현 → `hold` 또는 `rejected: genre_dependent`
   - 본진 중복? `merged`로 처리하고 본진 항목의 시그니처 예문에 합류
   - 처방 4대 철칙(의미 불변·근거 기반·장르 유지·과윤문 금지) 비충돌? 위반이면 `rejected: subjective_aesthetic` 또는 `ambiguous_overlap`
   - 위 모두 통과 → **승격**: 본진에 새 ID 발급(해당 대분류의 최하위 번호 + 1, 삽입 금지), 심각도·정의·예문·처방 작성, taxonomy.md "버전 관리" 섹션에 v1.x 항목 추가
